    ...
    return {
        ...
        "intelligence_layers": {
            "descriptive": {
                ...
                "economic_loss_inr": target_pis['economic_loss_inr'] if target_pis else None
            },
            "predictive": {
                "risk_1h": target_pred['forecasts']['1h']['hotspot_probability'] if target_pred else None,
                "risk_24h": target_pred['forecasts']['24h']['hotspot_probability'] if target_pred else None,
                "trend_24h": target_pred['forecasts']['24h']['trend_direction'] if target_pred else None
            },
            "prescriptive": {
                "best_simulation": best_dt['scenario_name'] if best_dt else None,
                "expected_csi_reduction": best_dt['csi_improvement'] if best_dt else None,
                "roi_score": best_dt['roi'] if best_dt else None
            },
            "operational": {
                "recommended_team": target_sep['recommended_team'] if target_sep else None,
                "recommended_time": target_sep['recommended_time'] if target_sep else None,
                "action": target_sep['recommended_action'] if target_sep else None
            },
            "root_cause_dna": {
