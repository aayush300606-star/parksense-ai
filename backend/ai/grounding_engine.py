...
class GroundingEngine:
    ...
    def validate_response(response_text: str, context: Dict[str, Any]) -> Tuple[bool, str]:
        ...
        safe_terms = ['Parking Impact Score', 'Economic Loss', 'Emissions', 'Network Fragility Score', 'Predictability Level', 'Long-term fix', 'The', 'Deploy', 'Prioritize', 'Overnight Residential Parking Overflow', 'Commercial', 'Freight Loading', 'Medium', 'High', 'Low', 'Rapid Towing Dispatch', 'Increased Frequency Patrols', 'Residential']
        potential_locations = GroundingEngine.extract_locations(response_text)
        for loc in potential_locations:
            if loc in safe_terms or loc in context_str:
                continue
