...
class LLMEngine:
    ...
    def generate_response(query: str, intent: str, context: Dict[str, Any]) -> Dict[str, Any]:
        ...
        if intent == "HOTSPOT_INQUIRY":
            ...
            nodes = context.get('network_csi_map', [])
            if nodes:
                n1 = nodes[0]
                response = f"The most vulnerable node in the traffic network is {n1.get('location_name', 'Unknown')}, which propagates delays with a ripple effect score of {n1.get('ripple_effect_score', 0)}."
                evidence.append(f"Network Fragility Score: {n1.get('network_fragility', 0):.1f}.")
                recommendations.append(f"Prioritize relieving {n1.get('location_name', 'Unknown')} to prevent cascading gridlock.")
            else:
                response = "Network intelligence data is unavailable."

        elif intent == "ROOT_CAUSE_INQUIRY":
            causes = context.get('top_root_causes', [])
            if causes:
                c1 = causes[0]
                response = f"The root cause of congestion is '{c1.get('primary_cause', 'Unknown')}', heavily influenced by '{c1.get('secondary_cause', 'Unknown')}'."
                evidence.append(f"Predictability Level: {c1.get('predictability', 'Unknown')}.")
                recommendations.append(f"Long-term fix: {c1.get('recommended_infrastructure_fix', 'Unknown')}.")
            else:
                response = "Root cause DNA is currently unavailable."
                
        elif intent == "PIS_INQUIRY":
            pis_zones = context.get('highest_impact_zones', [])
            if pis_zones:
                z = pis_zones[0]
                response = f"The highest Parking Impact Score (PIS) is {z.get('pis_score', 0):.1f} at {z.get('road_name', 'Unknown')}."
                evidence.append(f"Economic Loss: INR {z.get('economic_burden_inr_per_day', 0):.0f} per day. Emissions: {z.get('co2_emissions_kg_per_day', 0):.0f} kg CO2.")
                recommendations.append(f"Deploy enforcement at {z.get('road_name', 'Unknown')} to mitigate economic and environmental damage.")
            else:
                response = "Parking Impact Score data is currently unavailable."
