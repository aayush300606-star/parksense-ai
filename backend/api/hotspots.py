...
def get_legacy_hotspots(timeWindow: str = "today", severity: str = "Critical,High,Medium,Low"):
    """
    Returns the legacy JSON structure expected by the Next.js frontend,
    populated with real, un-mocked data.
    """
    return HotspotService.generate_frontend_compatible_hotspots(time_window=timeWindow, severity_levels=severity)
