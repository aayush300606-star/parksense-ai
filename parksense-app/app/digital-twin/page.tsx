        ...
        <div className="saas-card overflow-hidden flex flex-col h-full">
          ...
          <div className="p-6 flex-1 flex flex-col">
            <div className="w-full h-[200px] bg-slate-50 rounded-xl border border-slate-200 flex flex-col items-center justify-center mb-6 shadow-inner relative overflow-hidden">
               ...
               <div className="mt-2 text-sm text-slate-400">Current Speed: {baseline?.current_speed_kmh?.toFixed(1) || 0} km/h</div>
            </div>

            <div className="grid grid-cols-2 gap-4 mt-auto">
              <div className="bg-slate-50 p-5 rounded-xl border border-slate-200">
                <div className="text-sm font-medium text-slate-500 mb-1">Baseline CSI</div>
                <div className="text-3xl font-bold text-slate-900">{baseline?.csi_score?.toFixed(1) || "..."}</div>
              </div>
              <div className="bg-slate-50 p-5 rounded-xl border border-slate-200">
                <div className="text-sm font-medium text-slate-500 mb-1">Baseline PIS</div>
                <div className="text-3xl font-bold text-slate-900">{baseline?.pis_score?.toFixed(1) || "..."}</div>
              </div>
            </div>
          </div>
        </div>
        ...
        <div className="saas-card overflow-hidden flex flex-col h-full border-emerald-200 ring-1 ring-emerald-500/10 shadow-[0_4px_20px_-4px_rgba(16,185,129,0.1)]">
          ...
          <div className="p-6 flex-1 flex flex-col">
            <div className="w-full h-[200px] bg-emerald-50/50 rounded-xl border border-emerald-100 flex flex-col items-center justify-center mb-6 relative overflow-hidden shadow-inner">
               ...
               <div className="mt-2 text-sm text-emerald-600/70">Projected Speed: {simulated?.current_speed_kmh?.toFixed(1) || 0} km/h (+{baseline?.current_speed_kmh ? ((deltas?.speed_kmh?.improvement / baseline.current_speed_kmh) * 100).toFixed(0) : 0}%)</div>
            </div>

            <div className="grid grid-cols-2 gap-4 mt-auto">
              <div className="bg-white p-5 rounded-xl border border-emerald-100 shadow-sm">
                <div className="text-sm font-medium text-slate-500 mb-1">Projected CSI</div>
                <div className="text-3xl font-bold text-slate-900">{simulated?.csi_score?.toFixed(1) || "..."}</div>
              </div>
              <div className="bg-emerald-500 p-5 rounded-xl shadow-sm text-white">
                ...
                <div className="text-3xl font-bold">-{deltas?.csi?.improvement?.toFixed(1) || 0} pts</div>
              </div>
            </div>
          </div>
        </div>
            ...
            <tbody className="bg-white divide-y divide-slate-100">
              {currentHotspot?.scenarioList?.map((sim: any, i: number) => (
                <tr key={i} className={`transition-colors ${sim.scenario_id === selectedScenarioId ? 'bg-blue-50' : 'hover:bg-slate-50'}`}>
                  ...
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{sim.baseline_state.csi_score.toFixed(1)}</td>
                  <td className="px-6 py-4 whitespace-nowrap">
                    <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-emerald-100 text-emerald-800">
                      {sim.simulated_state.csi_score.toFixed(1)}
                    </span>
                  </td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">+{sim.baseline_state.current_speed_kmh ? ((sim.deltas.speed_kmh.improvement / sim.baseline_state.current_speed_kmh) * 100).toFixed(0) : 0}%</td>
                  <td className="px-6 py-4 whitespace-nowrap text-sm text-slate-500">{sim.benefit_score.toFixed(0)}</td>
